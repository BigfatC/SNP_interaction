...
def main():
    ...
    for oneline in inputfile:
        ...
        if (TypeofFile == 1):
            ...
            re1 = SNPID2POS(tempid[0])
            re2 = SNPID2POS(tempid[1])
            print(">{0}\t{1}".format(re1,re2),file= Output)
            gene1 = region2feature(re1,ext)
            gene2 = region2feature(re2,ext)

        else:
            region = RegionFormat(oneline)
            print('>{0}\t{1}'.format(region[1],region[2]),file= Output)
            gene1 = region2feature(region[0],ext)
            gene2 = region2feature(region[1],ext)

        if(gene1 != None and gene2 != None):
            gene2generelate(gene1,gene2)
            pathwayontology(gene1,gene2)
        else:
            print("No-gene-correlation-results\nNo-gene-correlation-results",file= Output)
